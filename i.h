...
namespace mgt {
template <size_t l = 1024> class i : public basic_uint<l> {
public:
  i() { this->data = 0; }
  ...
  explicit i(const std::string str) { this->from_string(str); }

  explicit i(std::bitset<l> s) { this->data = s; }
  ...
  friend bool operator<(const i<l> &a, const i<l> &b) { return is_neg(a - b); }

  friend bool operator<=(const i<l> &a, const i<l> &b) {
    return a < b || a == b;
  }

  friend bool operator>(const i<l> &a, const i<l> &b) { return !(a <= b); }
  ...
  friend bool is_neg(const i<l> &num) { return num.data[l - 1]; }
};
